...
def load_documentos() -> pd.DataFrame:
    df = _read_tab("Documentos_Cargados")
    if not df.empty:
        for col in ["num_transacciones", "total_gastos", "total_ingresos"]:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
        df["fecha_importacion"] = pd.to_datetime(df["fecha_importacion"], errors="coerce")
        df["fecha_desde"] = pd.to_datetime(df["fecha_desde"], errors="coerce")
        df["fecha_hasta"] = pd.to_datetime(df["fecha_hasta"], errors="coerce")
        df = df.sort_values("fecha_importacion", ascending=False)
    return df
